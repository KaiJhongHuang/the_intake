...
### ⚠️ 日期決定流程（執行 Prompt 時必須遵守）

1. **先確認當下台灣時間**：用系統提供的 `currentDate` 或其他方式取得今天日期
2. **該日期即為日報日期**：不要用「最近搜尋到的新聞日期」或「前一天」來反推
3. **常見錯誤**：系統標示今天為 4/7 → 應產出 **4/7 日報**（涵蓋 4/6 06:00 ~ 4/7 06:00），而非 4/6 日報
